            ...
            <form action="{{ route('admin.apartments.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group form-create services-form">
                    <ul class="services-ul">
                        ...
                        <li class="list-type">
                            <input style="margin-right: 15px;" @if (in_array($service->id, old('services',
                            $apartment_services_id))) checked @endif type="checkbox" name="services[]"
                            value="{{ $service->id }}" id="{{ $service }}">
                    ...
                    <div class="margin-top-forms">
                        ...
                        <div class="form-group">
                            <input placeholder="Metri quadrati" required class="form-control little-forms" type="number"
                                name="mq2" id="mq2" min="1" value="{{ old('mq2', $apartment->mq2) }}" />
                            @error('mq2')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group margin-top-services">
                            <input placeholder="N° di stanze" required class="form-control little-forms" type="number"
                                name="rooms" id="rooms" min="1" value="{{ old('rooms', $apartment->rooms) }}" />
                            @error('rooms')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="margin-top-forms">
                        ...
                        <div class="form-group">
                            <input placeholder="N° di letti" required class="form-control little-forms" type="number"
                                name="beds" id="beds" min="1" value="{{ old('beds', $apartment->beds) }}" />
                            @error('beds')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="form-group radio">
                    ...
                    <div class="select-visible">
                        <select class="select-form-visible" name="cars" id="cars">
                            <option name="is_visible" id="is_visible" value="1" @if ($apartment->is_visible) checked
                                @endif>Sì</option>
                            <option name="is_visible" id="is_visible" value="0" @if (!$apartment->is_visible) checked
                                @endif>No</option>
                        </select>
                    </div>
